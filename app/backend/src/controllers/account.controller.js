...
const accountController = {
  ...
  withdrawalById: async (req, res) => {
    try {
      ...
      if (account === 'negative') {
        return res.status(StatusCode.NOT_FOUND).json({ message: 'Saldo insuficiente para sacar' });
      }

      if (account === 'invalid') {
        return res.status(StatusCode.NOT_FOUND).json({ message: 'Formato de entrada invalido' });
      }
